...
function playRound(playerChoice, computerChoice = getComputerChoice()) {
  ...
  dom.get(
    'p2'
  ).textContent = `You chose ${playerChoice}. Computer chose: ${computerChoice}`;

  let [playerScore, cpuScore] = [
    state.get('playerScore'),
    state.get('cpuScore'),
  ];
  ...
  function outcomeMessage(result = '') {
    if (result.includes('win')) {
      ...
      result = 'won';
      return `Round ${roundCount}/5 ${result}! ${
...
function init() {
  ...
  document
    .querySelector('.div1')
    .append(document.querySelector('.div2'), document.querySelector('.div3'));

  // Set dom proprties to querySelected elements
  elArray.forEach(
    (element) =>
      (dom._vars[element.className] = document.querySelector(
        `.${element.className}`
      ))
  );
